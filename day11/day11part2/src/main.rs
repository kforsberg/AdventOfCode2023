    ...
    for (row_index, row) in grid.iter().enumerate() {
        for (column_index, chr) in row.iter().enumerate() {
            if *chr == '#' {
                points.insert(
                    points.len(),
                    get_expanded_coordinates(column_index, row_index, empty_rows, empty_columns, 1000000),
                );
            }
        }
    }
...
fn get_expanded_coordinates(
    ...
    let empty_columns: Vec<usize> = empty_columns
        .iter()
        .filter(|&&index| index < x)
        .map(|f| *f)
        .collect();

    let empty_rows: Vec<usize> = empty_rows
        .iter()
    ...
    Point::new(
        x as u64 + (empty_columns.len() as u64 * (expansion_factor - 1)),
        y as u64 + empty_rows.len() as u64 * (expansion_factor - 1),
    )
}
